...
class FileSystem : IFileSystem {
    ...
    private fun allocateBlockToRecord(record: FileRecord, number: Int): Boolean {
        ...
        val dValue: Int =
            afterSingle - beforeSingle + afterDouble - beforeDouble + afterTriple - beforeTriple //计算由于分配指针带来的额外块分配数
    ...
    private fun modifyRecordDataPointer(record: FileRecord, emptyBlockIndex: Int) {
        if (record.fileOccupiedBlock < 12) { //
            ...
            modifyRecordSingleIndirectPointer(
                record.singleIndirectPointer,
                record.fileOccupiedBlock - 12,
                newEmptyBlockIndex
            )
            ...
            modifyRecordDoubleIndirectPointer(
                record.doubleIndirectPointer,
                record.fileOccupiedBlock - 268,
                newEmptyBlockIndex
            )
            ...
            val newEmptyBlockIndex: Int = findFirstEmptyDataBlockIndex()
            flipDataBitmapBit(newEmptyBlockIndex)
            modifyRecordTripleIndirectPointer(
                record.tripleIndirectPointer,
                record.fileOccupiedBlock - 65804,
                newEmptyBlockIndex
            )
        } else { //
            modifyRecordTripleIndirectPointer(
    ...
    override fun fileRead(handle: FileHandle, dst: ByteArray, byteNumber: Int): Int {
        ...
        if (record.fileSize - fileDescriptorList[handle].rwPointer >= byteNumber) { //可以读出byteNumber字节
            ...
            val endOffset: Int =
                (endBlock - beginBlock) * 1024 + (fileDescriptorList[handle].rwPointer + byteNumber - 1) % 1024 //右闭
